...
import { generateCSSShorthand, isVisible } from "@shared/common";
import { hasValue, isStrokeWeights } from "@shared/identity";
...
export function buildSimplifiedStrokes(
    ...
    } else if (hasValue("strokeWeight", n) && typeof n.strokeWeight === "number" && n.strokeWeight > 0) {
...
export function parsePaint(raw: Paint, hasChildren: boolean = false): SimplifiedFill {
    if (raw.type === "IMAGE") {
        ...
        if (raw.imageTransform) {
            ...
            finalProcessing = {
                ...
                requiresImageDimensions:
                    processing.requiresImageDimensions ||
                    transformProcessing.requiresImageDimensions,
            };
        }
        ...
        if (!raw.color) {
            throw new Error("SOLID paint type requires a color");
        }
        const { hex, opacity } = convertColor(raw.color, raw.opacity);
        return opacity === 1 ? hex : formatRGBAColor(raw.color, opacity);
...
function mapLinearGradient(
    ...
    if (extendedIntersections.length >= 2) {
        ...
        const mappedStops = gradientStops.map(({ position, color }) => {
            ...
            const lineLength = fullLineEnd - fullLineStart;
            if (lineLength === 0) {
                return `${cssColor} ${Math.round(figmaLinePosition * 100)}%`;
            }
            const extendedPosition = (figmaLinePosition - fullLineStart) / lineLength;
